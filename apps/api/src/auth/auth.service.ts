...
export class AuthService {
  ...
  async signup(userData: {
    ...
    if (userData.avatar) {
      ...
      const avatarUrl = await this.uploadBase64ImageToS3(userData.avatar, AVATAR_FOLDER);
  ...
  private validateBase64Image(base64Image: string): void {
    ...
    if (error) {
      throw new BadRequestException('Invalid base64 image or image size exceeds 5MB');
    }
  }

  private async uploadBase64ImageToS3(base64Image: string, folder: string): Promise<string> {
    const buffer = Buffer.from(base64Image.replace(/^data:image\/\w+;base64,/, ''), 'base64');
    const key = `${folder}/${uuidv4()}.jpg`;

    const uploadResult = await this.awsService.uploadFile(buffer, key, 'image/jpeg');
    return uploadResult;
  }
  ...
  async updateAvatar(userId: string, data: UpdateAvatarInput): Promise<User> {
    ...
    const avatarUrl = await this.uploadBase64ImageToS3(data.base64Image, AVATAR_FOLDER);
